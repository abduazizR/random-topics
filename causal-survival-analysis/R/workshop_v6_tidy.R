...
summary(cox_fit)
cox_fit |> gtsummary::tbl_regression(exponentiate = T)
...
exp(coeftest(plr_fit, vcov=vcovHC(plr_fit, type="HC1"))) # To get robust SE estimates
...
summary(adj_cox_fit)
adj_cox_fit |> gtsummary::tbl_regression(exponentiate = T)

# Calculate the baseline covariate-adjusted hazard ratio from a pooled logistic regression model
adj_plr_fit <- glm(death ~ visit + I(visit^2) + rand + mi_bin + NIHA_b + HiSerChol_b +
                     HiSerTrigly_b + HiHeart_b + CHF_b + 
                     AP_b + IC_b + DIUR_b + AntiHyp_b + 
                     OralHyp_b + CardioM_b + AnyQQS_b + 
...
adj_plr_fit |> 
  gtsummary::tbl_regression(tidy_fun = partial(tidy_robust, vcov = "HC1"), exponentiate = T)
...
# Load the data from the trial
trial_full <- read.csv("causal-survival-analysis/R/trial1.csv", header=TRUE)
